...
func Parse(payload envelope.Data) (channels.Message, error) {
	...
	if len(payload.ProtocolIDs) != 1 {
		return nil, errors.Wrapf(channels.ErrInvalidMessage, "teller messages can't wrap")
	}
	...
	if version != 0 {
		return nil, errors.Wrap(channels.ErrUnsupportedVersion, fmt.Sprintf("teller %d", version))
	}
	...
	if result == nil {
		return nil, errors.Wrap(channels.ErrNotSupported,
